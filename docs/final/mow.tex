...
Funkcj¹ oceny jest b³¹d œredniokwadratowy:
...
Ostatnim zbiorem danych s¹ notowania 10 spó³ek gie³dowych
(plik \verb|data/stockprices.data|). Atrybutem docelowym jest
cena spó³ki numer 10. W zbiorze znajduje siê 950 przyk³adów.
...
Niestety, ju¿ w momencie uczenia na 2/3 zbioru daje siê odczuæ powolnoœæ algorytmu ucz¹cego. Wyniki testu s¹ nastêpuj¹ce:
...
Dla takich zbiorów danych, zaprojektowany aproksymator okazuje siê zupe³nie 
nieprzydany. Proces uczenia siê jest bardzo wolny, a nauczony aproksymator podczas predykcji aktywuje jedynie czêœæ wag które by³y ustawiane podczas
uczenia siê. Powoduje to astronomiczny b³¹d aproksymatora. Mo¿e to mieæ zwi¹zek 
miêdzy innymi z nierównomiernym rozmieszczeniem przyk³adów jeœli chodzi o
dziedzinê atrybutów.

\subsubsection{Uwagi}
Przeprowadzaliœmy próby na wiêkszych zbiorach danych (jeœli chodzi o 
iloœæ przyk³adów i atrybutów) jednak okaza³o siê, ¿e praktycznie niewykonalne
jest uczenie na zbiorach danych wiêkszych ni¿ gie³da. Z tego te¿ powodu
badanie wiêkszych zbiorów zosta³o zarzucone.
...
to pojawienie siê du¿ego b³êdu na wyjœciu aproksymatora. Sytuacja ta ma
...
liczby podzia³ów pierwszej warstwy wszystkich atrybutów w po³¹czeniu z liczb¹ warstw. Ponadto szybkoœæ dzia³ania aproksymatora, szczególnie podczas uczenia siê, pozostawia wiele do ¿yczenia. Jednak wszystkie te modyfikacje wykraczaj¹ poza zakres projektu.
\bibliography{mow}
\end{document}
